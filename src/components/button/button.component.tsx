...
const Button = ({ children, loading, ...rest }: IButtonProps) => {
  if (rest.outline) {
    return (
      <motion.button
        {...rest}
  ...
  return (
    <motion.button
      {...rest}
      className={`relative cursor-pointer flex justify-center items-center min-w-[5rem] px-[5.7rem] py-[1.5rem] rounded-full font-[inherit] ${
        rest.disabled ? 'button-gradient-no-hover' : 'button-gradient'
